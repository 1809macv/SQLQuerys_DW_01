...
SET @MinVisitDate = cast((SELECT min(VisitDate) FROM [PIVOT].extPsVisit) as DATE)
...
UPDATE [PIVOT].Efectividad
   SET Efectividad.PDVCompraron = (Select count(*) From [PIVOT].extEfectividad_Paso02 Compraron
                                    Where Compraron.DistribuidorId = Efectividad.IdDistribuidor and
										  Compraron.VendedorId = Efectividad.VendedorId and
										  Compraron.FechaVenta = Efectividad.Fecha );
...
-- Cliente en el mismo dia.
WITH Promedio2(DistribuidorId, Distribuidor, VendedorId, VendedorNombre, ClienteNombre, Fecha)
AS
(
