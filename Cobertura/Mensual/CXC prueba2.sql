...
SELECT @strQuery = 'SELECT IdDistribuidor, Distribuidor, Ciudad, AnioVenta, MesVenta, MesNumero, TipoNegocio, TipoCliente, CategoriaCliente, Zona, 
				CustomerId, ClienteNombre
  FROM OPENROWSET(''SQLNCLI11'',''Server=dbmsfsystem.database.windows.net;UID=msfadmin;PWD=arcor.123*;Database=MSFSystemVacio'', ''SELECT DISTINCT Sale.CompanyId as IdDistribuidor
		 ...
		 WHERE Sale.StatusIdc <> 104  and Sale.CompanyId >= 3 
			   AND CAST(SaleDate as DATE) > ' + CONVERT(varchar(10),@FechaDesde) + ' AND CAST(SaleDate as DATE) < ' + CONVERT(varchar(10),@FechaHoy)
