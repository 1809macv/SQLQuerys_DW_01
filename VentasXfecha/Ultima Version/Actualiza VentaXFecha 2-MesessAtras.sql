	...
	BEGIN
		...
		SELECT Cmp.Id as DistribuidorID 
		  ...
		  FROM MSFSystemVacio.[Sales].MsSale Sale 
			   ...
			   INNER JOIN MSFSystemVacio.[Warehouse].[PsOpc] Opc ON Opc.Id = OpcSale.OpcId 
		...
		(Distribuidor, NombreContacto ,Ciudad ,AnioVenta ,MesVenta ,FechaVenta ,Zona ,ClienteTipo ,Categoria ,TipoRecursivo      ---,DiaVenta 
		 ...
		 SELECT 
				Distribuidor ,NombreContacto ,Ciudad, AnioVenta, MesVenta, FechaVenta, Zona, ClienteTipo, Categoria, TipoRecursivo, ClienteCodigo, RazonSocial      ---DiaVenta, 
			   ,ClienteNombre, NumeroNit, NumeroFactura, AutorizacionNumero, CodigoControl, TipoPago, DocumentoEstado, Vendedor, Almacen, Negocio 
			   ,Reclasificacion, Segmento, SubRubro, ProductoCodigo, ProductoNombre, Bonificacion, NumeroLote, UnidadMedidaVenta, FactorConversion 
		  ...
		  WHERE DistribuidorID >= 3
				AND( FechaVenta >= @FechaDesde AND FechaVenta < @FechaHoy) 
