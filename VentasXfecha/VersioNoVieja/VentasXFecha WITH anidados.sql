WITH Producto ( ProductoId, ProductoCodigo, ProductoNombre, PesoNeto, PesoBruto, Negocio, Reclasificacion, Segmento, SubRubro )
AS
(
SELECT Prd.Id AS ProductoId
...
Cliente ( ClienteId, ClienteCodigo, ClienteNombre, Direccion, NumeroTelefono, Zona, ClienteTipo, Categoria, TipoRecursivo )
AS 
(
SELECT Customer.Id as ClienteId 
	  ,Customer.Code AS ClienteCodigo ,Customer.Name AS ClienteNombre 
	  ,Customer.Address as Direccion
	  ,Customer.Phone as NumeroTelefono
	  ,Zne.Name AS Zona ,C9.Name AS ClienteTipo ,C10.Name AS Categoria 
...
--WITH 
Ventas (DistribuidorID, Distribuidor, NombreContacto, Ciudad, AnioVenta, MesVenta, FechaVenta, Zona, ClienteTipo, Categoria, TipoRecursivo, 
...
AS (
	SELECT Cmp.Id as DistribuidorID ,Cmp.[Name] AS Distribuidor ,Cmp.Contact as NombreContacto ,C8.[Name] AS Ciudad ,Year(Sale.SaleDate) AS AnioVenta
		  ,Month(Sale.SaleDate) AS MesVenta ,Convert(date,Sale.SaleDate) AS FechaVenta ,Customer.Zona ,Customer.ClienteTipo ,Customer.Categoria
		  ,Customer.TipoRecursivo ,Customer.ClienteCodigo ,Sale.CustomerName AS RazonSocial ,Customer.ClienteNombre
		  ...
		  ,C1.[Name] AS TipoPago ,C2.[Name] AS DocumentoEstado ,Usr.[Name] AS Vendedor ,Store.[Name] AS Almacen ,Prd.Negocio ,Prd.Reclasificacion
		  ,Prd.Segmento ,Prd.SubRubro ,Prd.ProductoCodigo ,Prd.ProductoNombre 
		  ,CASE SaleD.Bonus
					WHEN 0 THEN 'No'
					WHEN 1 THEN 'Si'
		   END AS Bonificacion
		  ...
		  ,CASE SaleD.DiscountTypeIdc WHEN 33 THEN Round(((SaleD.Total*SaleD.Discount*100)/10000),2) WHEN 34 THEN SaleD.Discount
		   ...
		   ELSE 0.00 END AS DescuentoDocumento ,Usr1.[Name] AS UsuarioTransaccion ,Prd.PesoNeto ,Prd.PesoBruto
		  ,(Prd.PesoNeto*SaleD.Quantity/CUnit.Equivalence) AS TotalKilosNetos ,(Prd.PesoBruto*SaleD.Quantity/CUnit.Equivalence) AS TotalKilosBrutos
...
FROM Sales.MsSale Sale
			...
			INNER JOIN Producto Prd ON Prd.ProductoId = SaleD.ProductId
			...
			INNER JOIN Cliente Customer ON Customer.ClienteId = Sale.CustomerId
			INNER JOIN Warehouse.PsCompanyUnit CUnit ON CUnit.CompanyId = Cmp.Id and CUnit.ProductId = Prd.ProductoId
...
SELECT TOP 10000 DistribuidorID ,Distribuidor ,NombreContacto ,Ciudad, AnioVenta
	  ,CASE MesVenta
...
-- Recuperar los primeros 1000 registros demoro 31 seg
-- Mejora 1: Se creo una tabla Producto 
-- Mejora 2: Se creo una tabla Cliente 
