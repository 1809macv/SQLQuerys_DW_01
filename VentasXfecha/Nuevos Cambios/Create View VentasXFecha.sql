/****** Object:  View [PIVOT].[VentasXFecha]    Script Date: 10/4/2019 2:15:26 PM ******/
SET ANSI_NULLS ON
GO
...
WITH Ventas (DistribuidorID, Ciudad, FechaVenta, Zona, ClienteTipo, Categoria, TipoRecursivo, 
			 ...
			 UnidadMedidaVenta, FactorConversion, ListaPrecio, CantDISP_UN, UnidadMedidaAlmacen, CantBU, PrecioProducto, TotalProducto, DescuentoProducto, 
...
AS (
	SELECT Sale.CompanyId as DistribuidorID ,C8.[Name] AS Ciudad 
		  ...
		  ,SaleD.Equivalence AS FactorConversion 
		  ,PLst.[Name] AS ListaPrecio
		  ,SaleD.Quantity AS CantDISP_UN 
		  ,C12.[Name] as UnidadMedidaAlmacen 
		  ,(SaleD.Quantity/SaleD.Equivalence) AS CantBU 
		  ,SaleD.Price AS PrecioProducto ,SaleD.Total AS TotalProducto
		  ...
		  ,(Prd.Weight*SaleD.Quantity/SaleD.Equivalence) AS TotalKilosNetos 
		  ,(Prd.TotalWeight*SaleD.Quantity/SaleD.Equivalence) AS TotalKilosBrutos
		  ,(STd.PPP / SaleD.Equivalence) AS CostoPrecioPromedio 
		  ,(SaleD.Quantity*(STd.PPP/SaleD.Equivalence)) as TotalCostoPromedio
		  ...
		  ,convert(date, Opc.CreationDate) as FechaCreacionGuia ,convert(date, Opc.DispatchDate) as FechaDespachoGuia
  		  ,(Select Observation From [Accounting].MsReceivable Recei Where Recei.SaleId = Sale.id) as Descripcion 
	  FROM Sales.MsSale Sale
			...
			INNER JOIN Base.PsClassifier C12 ON C12.Id = SaleD.StorageUnitIdc
	...
	SELECT Sale.CompanyId as DistribuidorID ,C8.[Name] AS Ciudad 
		  ...
		  ,SaleD.Quantity AS CantDISP_UN 
  		  ,C12.[Name] as UnidadMedidaAlmacen 
		  ,(SaleD.Quantity/SaleD.Equivalence) AS CantBU ,SaleD.Price AS PrecioProducto ,SaleD.Total AS TotalProducto
		  ,CASE SaleD.DiscountTypeIdc WHEN 33 THEN Round(((SaleD.Total*SaleD.Discount*100)/10000),2) WHEN 34 THEN SaleD.Discount
		   ELSE 0.00 END AS DescuentoProducto ,CASE Sale.DiscountTypeIdc WHEN 33 THEN (Sale.Discount/100) WHEN 34 THEN Round((Sale.Discount/Sale.Total),6)
		   ELSE 0.00 END AS DescuentoDocumento ,Usr1.Name AS UsuarioTransaccion ,Prd.Weight AS PesoNeto ,Prd.TotalWeight AS PesoBruto
...
SELECT TOP 10 DistribuidorID ,Ciudad
	  ...
	  ,FactorConversion ,ListaPrecio ,CantDISP_UN 
	  ,UnidadMedidaAlmacen
	  ,CantBU ,PrecioProducto ,TotalProducto ,DescuentoProducto 
