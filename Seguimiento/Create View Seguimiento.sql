ALTER VIEW [PIVOT].Seguimiento
AS

SELECT top 10 Pedido.CompanyId as DistribuidorId
	  , Comp.[Name] As Distribuidor
	  , CR.[Sequence] as Secuencia
	  , Cliente.Code as ClienteCodigo 
	  , Pedido.CustomerName as ClienteNombre
	  , convert(date, Pedido.InitialDate) as Fecha
	  , convert(time, Pedido.InitialDate, 108) as HoraInicio
	  , convert(time, Pedido.FinalDate, 108) as HoraFin
  ...
  FROM Sales.MsOrder Pedido
		...
		INNER JOIN [Sales].[MsCustomer] Cliente On Cliente.Id = Pedido.CustomerId
...
UNION ALL 
SELECT Visita.CompanyId as DistribuidorId
	  , Comp.[Name] as Distribuidor
	  , CR.[Sequence] as Secuencia
	  , Cliente.Code as ClienteCodigo 
	  , Cliente.[Name] as ClienteNombre
	  , convert(date, VisitDate) as Fecha
	  , convert(time, VisitDate, 108) as HoraInicio
	  , convert(time, VisitDate, 108) as HoraFin
...
SELECT Pedido.CompanyId as DistribuidorId
	  , Comp.[Name] As Distribuidor
	  , 0 as Secuencia
	  , Cliente.Code as ClienteCodigo 
	  , Pedido.CustomerName as ClienteNombre
